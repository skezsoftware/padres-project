...
export const SwingMissPlot = ({ data, pitchType, usage, avgVelo, avgSpin, whiff_pct, isHighest, color }) => {
  const svgRef = useRef()
  const width = 250
  const height = 320
  ...
  useEffect(() => {
    ...
    // Create a density heatmap with swing-and-miss data
    ...
    homeplatePath.moveTo(xScale(0), yScale(-0.4));
    homeplatePath.lineTo(xScale(0.708), yScale(0));
    homeplatePath.lineTo(xScale(0.708), yScale(0.4));
    homeplatePath.lineTo(xScale(-0.708), yScale(0.4));
    homeplatePath.lineTo(xScale(-0.708), yScale(0));
    homeplatePath.closePath();
    ...
    // Add scatter points for swing and miss only
  ...
  return (
    <div className={`scatter-plot-container ${isHighest ? 'highest' : ''}`}>
      <div className="pitch-info">
        <h3 className="pitch-type" style={{ color: color }}>Whiffs: {pitchType}</h3>
        <p className="pitch-stats">

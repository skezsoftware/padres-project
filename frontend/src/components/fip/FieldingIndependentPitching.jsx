...
export const FieldingIndependentPitching = ({ teamId, filters }) => {
    ...
    const fetchData = async () => {
      try {
        const queryParams = new URLSearchParams({
          ...
          sort_by: 'fip',
          order: 'asc',
          min_innings: '0.1',  // Add minimum innings requirement
        ...
        if (result.success) {
          ...
          const validData = result.data.filter(d => 
            d.fip !== undefined && 
            d.fip !== null && 
            isFinite(d.fip) &&
            d.innings_pitched > 0
  ...
  useEffect(() => {
    ...
    // Create color scale (inverted since lower FIP is better)
    const colorScale = d3.scaleLinear()
      .domain([d3.min(data, d => d.fip), d3.max(data, d => d.fip)])
      .range(['#0b5394', '#cfe2f3']); // Dark blue to light blue (inverted)

    // Create scales with adjusted x domain for negative values
    const x = d3.scaleLinear()
      .domain([Math.min(0, d3.min(data, d => d.fip)), d3.max(data, d => d.fip)])
      .range([0, width]);
    ...
    // Add Y axis with improved styling and larger font
    svg.append('g')
      .call(d3.axisLeft(y))
      .selectAll('text')
      .style('font-size', '18px')
    ...
    // Add background rectangle for above average (light red) - right side
    svg.append('rect')
      .attr('x', x(mlbAverage))
      .attr('y', 0)
    ...
    // Add background rectangle for below average (light green) - left side
    svg.append('rect')
      .attr('x', x(Math.min(0, d3.min(data, d => d.fip))))  // Start from minimum FIP value
      .attr('y', 0)
      .attr('width', x(mlbAverage) - x(Math.min(0, d3.min(data, d => d.fip))))
      .attr('height', height)
    ...
    svg.selectAll('rect.bar')
      ...
      .on('mouseover', function(event, d) {
        ...
        const adjustedXPosition = xPosition + boxWidth > width ? 
  ...
  return (
    <div className="fielding-independent-pitching" style={{ textAlign: 'center' }}>
